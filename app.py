...
def preprocessing(desease):
    ...
    Y = desease['중증질환']
    X = desease[['호흡 곤란', '간헐성 경련', '설사', '기침', '출혈', '통증', '만지면 아프다',
...
def predict_disease(patient_data):
    ...
    model_XGC = joblib.load('./xgb_model2.pkl')
...
with tab2: 
    ...
    day_list_df.rename(columns = {'datetime': '일별'}, inplace=True)
    ...
    group_day_time = group_day_time.rename(columns = {'이송 시간': '일별 이송 시간 평균'})
    st.line_chart(data=group_day_time, x='출동일시', y='일별 이송 시간 평균', use_container_width=True)
    ...
    dispatch_count = select_df.groupby(by = f'{select_bins}', as_index = False)['ID'].count()
    dispatch_count = dispatch_count.rename(columns = {'ID': '출동건수'})
    dispatch_count = dispatch_count.sort_values('출동건수')

    st.bar_chart(data = dispatch_count, x=select_bins, y='출동건수')
    ...
    with col240: # 성별 통계
        ...
        group_day_disease = group_day_disease.rename(columns={'ID':'환자수'})

        fig = px.pie(group_day_disease, values='환자수', names='성별', title=f'{select_bins} 성별 통계', hole=0.3, width=400, height=400)
    ...
    with col241: # 중증질환별 통계
        ...
        fig = px.pie(group_day_disease, values='환자수', names='중증질환', title=f'{select_bins} 중증질환 통계', hole=0.3, width=400, height=400)
        fig.update_traces(textposition='inside', textinfo = 'percent+label+value')
        fig.update_layout(font=dict(size=12))
        st.plotly_chart(fig)
    ...
    with col242:  # 나이대별 통계
        ...
        fig = px.pie(group_day_disease, values='환자수', names='나이대', title=f'{select_bins} 나이대 통계', hole=0.3, width=400, height=400)
        fig.update_traces(textposition='inside', textinfo = 'percent+label+value')
        fig.update_layout(font=dict(size=12))
        st.plotly_chart(fig)
